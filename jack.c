...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Amogh");
MODULE_DESCRIPTION("Kernel module to create a sorted linked list of child processes based on birthdays and their display memory segments");
...
static int birthday_thread(void *data) {
    ...
    printk(KERN_INFO "Kernel thread started for Birthday: %02d/%02d/%04d (PID %d)\n", node->day, node->month, node->year, node->pid);
    return 0;
}
